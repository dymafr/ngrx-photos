...
import { PhotosState } from '../../photos/shared/store/reducers';
  ...
  public getPictures(per_page: number = 30): Observable<Photo[]>  {
    return this.store.pipe(
      ...
      switchMap( (state: PhotosState) => {
        ...
        if (filter) {
          return fromPromise(this.unsplash.search.photos(filter, 0, per_page).then( res => res.json())).pipe(
            map( (res: any) => res.results)
          );
        } else {
          return fromPromise(this.unsplash.photos.listPhotos(0, per_page).then( result => result.json()));
        }
      })
    );
  }
